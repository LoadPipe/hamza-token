...
import "@baal/interfaces/IBaalToken.sol";

/// @title Baal ';_;'.
/// @notice Flexible guild contract inspired by Moloch DAO framework.
contract CustomBaal is Module, EIP712Upgradeable, ReentrancyGuardUpgradeable, BaseRelayRecipient {
    ...
    address public communityVault; /* Track the community vault address. */
    ...
    function setUp(bytes memory _initializationParams)
        ...
        nonReentrant
    {
        // Note the extra param: _communityVault
        (
            ...
            address _communityVault        /* <-- NEW: Community vault address parsed from initParams */
        ) = abi.decode(
            _initializationParams,
            (address, address, address, address, address, bytes, address)
        );
    ...
    *****************/
    ...
    function _ragequit(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            uint256 daoBalance;
            if (tokens[i] == ETH) {
                daoBalance = address(target).balance;
            } else {
